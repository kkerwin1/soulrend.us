...
if ( $product->is_on_sale() ) { ?>
	<span class="edgtf-<?php echo esc_attr( $class_name ); ?>-onsale"><?php echo playerx_edge_get_woocommerce_sale( $product ); ?></span>
<?php } ?>

<?php if ( ! $product->is_in_stock() ) { ?>
	<span class="edgtf-<?php echo esc_attr( $class_name ); ?>-out-of-stock"><?php esc_html_e( 'Sold', 'playerx' ); ?></span>
<?php }

$new = get_post_meta( get_the_ID(), 'edgtf_show_new_sign_woo_meta', true );

if ( $new === 'yes' ) { ?>
	<span class="edgtf-<?php echo esc_attr( $class_name ); ?>-new-product"><?php esc_html_e( 'New', 'playerx' ); ?></span>
<?php }
...
if ( $image_size === 'full' ) {
	...
	$product_image_size = 'playerx_edge_image_square';
} else if ( $image_size === 'landscape' ) {
	$product_image_size = 'playerx_edge_image_landscape';
} else if ( $image_size === 'portrait' ) {
	$product_image_size = 'playerx_edge_image_portrait';
...
if ( ! empty( $fixed_image_size_meta ) && isset( $type ) && $type === 'masonry' ) {
	if ( $image_size === 'small' ) {
		...
		$product_image_size = 'playerx_edge_image_huge';
	}
}

if ( has_post_thumbnail() ) {
	the_post_thumbnail( apply_filters( 'playerx_edge_filter_product_list_image_size', $product_image_size ) );
}