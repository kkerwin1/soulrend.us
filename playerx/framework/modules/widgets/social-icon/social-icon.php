...
class PlayerxEdgeClassSocialIconWidget extends PlayerxEdgeClassWidget {
	
	public function __construct() {
		parent::__construct(
			'edgtf_social_icon_widget',
			esc_html__( 'Playerx Social Icon Widget', 'playerx' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'playerx' ) )
		);
		...
		$this->params = array_merge(
			array(
				array(
					'type'  	  => 'dropdown',
					'name'  	  => 'icon_text',
					'title' 	  => esc_html__( 'Display Icon or Text', 'playerx' ),
					'description' => esc_html__( 'Choose whether you want to display social icon or text instead', 'playerx' ),
					'options' 	  => array(
						'icon' => esc_html__( 'Icon', 'playerx' ),
						'text' => esc_html__( 'Text', 'playerx' )
					)
				),
				array(
					'type'        => 'textfield',
					'name'        => 'social_text',
					'title'       => esc_html__( 'Social Text', 'playerx' ),
					'description' => esc_html__( 'Instert text which will describe desired social network', 'playerx' )
				)
			),
			playerx_edge_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		?>
		<a class="edgtf-social-icon-widget-holder edgtf-icon-has-hover" <?php echo playerx_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php playerx_edge_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
