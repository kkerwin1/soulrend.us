...
class PlayerxEdgeClassIconWidget extends PlayerxEdgeClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_icon_widget',
			esc_html__( 'Playerx Icon Widget', 'playerx' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'playerx' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			playerx_edge_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'playerx' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'playerx' ),
				...
				array(
					'type'  => 'textfield',
					'name'  => 'text_size',
					'title' => esc_html__( 'Text Size (px)', 'playerx' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'text_color',
					'title' => esc_html__( 'Text Color', 'playerx' )
				),
				array(
					'type'        => 'textfield',
					'name'        => 'margin',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? playerx_edge_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['text_size'] ) ) {
			$text_styles[] = 'font-size: ' . playerx_edge_filter_px( $instance['text_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_pack'] ) ) {
			...
			if ( ! empty( $icon_class ) ) {
				...
				$icon_holder_html = '<span class="edgtf-icon-element ' . esc_attr( $icon_class ) . '" ' . playerx_edge_get_inline_style( $icon_styles ) . '></span>';
			}
		}
		...
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<span class="edgtf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . playerx_edge_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
		}
		?>
		
		<a <?php playerx_edge_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo playerx_edge_get_inline_style( $holder_styles ); ?>>
